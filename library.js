...
window.setSpeed = function (pause) {
  ...
  if (stepByStep == true) {
    ...
    intervalID = 0;
  } else stepByStep = true;
  var intervalID = setInterval(function () {
    var command = steps.shift();
    var cc =
      command[0] + "(" + (command.length < 2 ? "" : command[1] + ",") + "true)";
    eval(cc);
    if (steps.length == 0) {
      clearInterval(intervalID);
      intervalID = 0;
...
window.forward = function (distance, step = false) {
  if (stepByStep && !step) {
    if (!isNaN(distance)) steps.push(["forward", distance]);
  } else {
    ...
    while (distance > 0) {
      if (pixelMode)
        imageContext.moveTo(
          x + (Math.cos(turtle.angle) * turtle.width) / 2,
          y - (Math.sin(turtle.angle) * turtle.width) / 2
        );
      else imageContext.moveTo(x, y);
      ...
      function xWrap(cutBound, otherBound) {
        ...
        if (pixelMode)
          imageContext.lineTo(
            cutBound + (Math.cos(turtle.angle) * turtle.width) / 2,
            edgeY - (Math.sin(turtle.angle) * turtle.width) / 2
          );
        else imageContext.lineTo(cutBound, edgeY);
      ...
      function yWrap(cutBound, otherBound) {
        ...
        if (pixelMode)
          imageContext.lineTo(
            edgeX + (Math.cos(turtle.angle) * turtle.width) / 2,
            cutBound - (Math.sin(turtle.angle) * turtle.width) / 2
          );
        else imageContext.lineTo(edgeX, cutBound);
      ...
      function noWrap() {
        if (pixelMode)
          imageContext.lineTo(
            newX + (Math.cos(turtle.angle) * turtle.width) / 2,
            newY - (Math.sin(turtle.angle) * turtle.width) / 2
          );
        else imageContext.lineTo(newX, newY);
...
window.waitSteps = function (nbSteps, step = false) {
  if (stepByStep && !step) {
    for (let i = 0; i < nbSteps; i++) steps.push(["waitSteps", 0]);
  } else {
  }
};
...
window.showGrid = function (step, isStep = false) {
  if (stepByStep && !isStep) {
    ...
    width(2);
    for (
      var x = -turtleCanvas.width / 2;
      x < turtleCanvas.width / 2;
      x += step
    ) {
      if (x == 0) width(2);
      else width(0.5);
    ...
    width(2);
    for (
      var y = -turtleCanvas.height / 2;
      y < turtleCanvas.height / 2;
      y += step
    ) {
      if (y == 0) width(2);
      else width(0.5);
...
function initialise() {
  ...
  if (msg === null && document?.body !== null) {
    ...
    msg.innerText =
      "Les coordonnées et la couleur ont été copiées dans le presse-papier";
  ...
  turtleCanvas.addEventListener("click", function (e) {
    // use event argument
    copyTextToClipboard(
      "(" +
        canvasX +
        ...
        canvasY +
        ") " +
        getPixelSimpleColor(canvasX, canvasY)
    );
    if (msg !== null) {
  ...
  turtleCanvas.addEventListener("mousemove", function (event) {
    ...
    canvasX = parseInt(
      pixelMode == true
        ? coords.x / pixelSize
        : coords.x - turtleCanvas.width / 2
    );
    canvasY = parseInt(
      pixelMode == true
        ? coords.y / pixelSize
        : -coords.y + turtleCanvas.height / 2
    );
    if (co !== null) {
      co.innerHTML =
        "X:" +
...
function draw() {
  ...
  if (turtle.visible) {
    var x = pixelMode
      ? turtle.pos.x + (Math.cos(turtle.angle) * turtle.width) / 2
      : turtle.pos.x;
    var y = pixelMode
      ? turtle.pos.y - (Math.sin(turtle.angle) * turtle.width) / 2
      : turtle.pos.y;
    var w = 10;
    var h = 15;
    turtleContext.save();
...
window.write = function (msg, isRelative = false, step = false) {
  if (stepByStep && !step) {
    ...
    centerCoords(imageContext);
    imageContext.translate(turtle.pos.x, turtle.pos.y);
    if (isRelative) imageContext.rotate(-turtle.angle);
    imageContext.transform(1, 0, 0, -1, 0, 0);
    imageContext.translate(-turtle.pos.x, -turtle.pos.y);
...
function RGBColor(color_string) {
  ...
  for (var key in simple_colors) {
    if (color_string == key) {
      color_string = simpleColorToSimpleColorTransparent(
        simple_colors[key],
        this.transparent
      );
    }
  }
  if (this.transparent == 0) color_string = "transparent";
  ...
  this.toSimpleColor = function () {
    ...
    if (this.transparent == 0) return "transparent";
    for (var key in simple_colors) {
...
function simpleColorToSimpleColorTransparent(color_string, transparent) {
  if (transparent < 255) {
    return (
      color_string +
      " opacity(" +
      Math.round((transparent / 255) * 100) / 100 +
      ")"
    );
  }
  return color_string;
...
function hexTransparentToHex(color_string) {
  ...
  if (color_string.length == 8) {
    ...
    if (
      !isNaN(transparent_number) &&
      transparent_number >= 0 &&
      transparent_number < 255
    ) {
